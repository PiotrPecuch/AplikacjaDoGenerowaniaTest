    ...
    filterList() {
      ...
      if (this.inputText !== '') {
        filtered = filtered.filter((item) =>
            item.examDescription.toLowerCase().includes(this.inputText.toLowerCase())
        );
      }
      ...
      if (this.pickedDate && this.pickedDate2===null) {
        filtered = filtered.filter((item) =>
            this.formatPickedDate(item.startDate) >= this.formatPickedDate(this.pickedDate)
        );
      }

      if (this.pickedDate2 && this.pickedDate===null) {
        filtered = filtered.filter((item) =>
            this.formatPickedDate(item.endDate) <= this.formatPickedDate(this.pickedDate2)
        );
      }

     if (this.pickedDate !== null && this.pickedDate2!==null) {
        filtered = filtered.filter((item) =>
            this.formatPickedDate(item.startDate) >= this.formatPickedDate(this.pickedDate) && this.formatPickedDate(item.endDate) <= this.formatPickedDate(this.pickedDate2)
        );
      }
